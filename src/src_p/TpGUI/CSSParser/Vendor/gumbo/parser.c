...
#define TAGSET_INCLUDES(tagset, ns, tag) ( \
    tag < GUMBO_TAG_LAST && (tagset[(int)tag] & (1 << (int)ns)))
...
{
  GumboNode *node = (GumboNode *)gumbo_malloc(sizeof(GumboNode));
...
{
  GumboOutput *output = (GumboOutput *)gumbo_malloc(sizeof(GumboOutput));
  output->root = NULL;
...
static void parser_state_init(GumboParser *parser)
{
  GumboParserState *parser_state =
      (GumboParserState *)gumbo_malloc(sizeof(GumboParserState));
...
static GumboInsertionMode get_current_template_insertion_mode(const GumboParser *parser)
{
  ...
  GumboInsertionMode *testModePtr = (GumboInsertionMode *)template_insertion_modes->data[(template_insertion_modes->length - 1)];
  return *testModePtr;
}
...
{
  gumbo_tagset testTagsetArray;
  testTagsetArray[GUMBO_TAG_MI] = (1 << GUMBO_NAMESPACE_MATHML);
  testTagsetArray[GUMBO_TAG_MO] = (1 << GUMBO_NAMESPACE_MATHML);
  testTagsetArray[GUMBO_TAG_MN] = (1 << GUMBO_NAMESPACE_MATHML);
  testTagsetArray[GUMBO_TAG_MS] = (1 << GUMBO_NAMESPACE_MATHML);
  testTagsetArray[GUMBO_TAG_MTEXT] = (1 << GUMBO_NAMESPACE_MATHML);

  //  TAG_MATHML(MI);
  //  TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS), TAG_MATHML(MTEXT) };
  return node_tag_in_set(node, testTagsetArray);
}
...
{
  gumbo_tagset testTagsetArray;
  testTagsetArray[GUMBO_TAG_FOREIGNOBJECT] = 1 << GUMBO_NAMESPACE_SVG;
  testTagsetArray[GUMBO_TAG_DESC] = 1 << GUMBO_NAMESPACE_SVG;
  testTagsetArray[GUMBO_TAG_TITLE] = 1 << GUMBO_NAMESPACE_SVG;

  return node_tag_in_set(node, testTagsetArray) ||
         (node_qualified_tag_is(node, GUMBO_NAMESPACE_MATHML, GUMBO_TAG_ANNOTATION_XML) && (attribute_matches(&node->v.element.attributes,
                                                                                                              "encoding", "text/html") ||
...
InsertionLocation get_appropriate_insertion_location(GumboParser *parser, GumboNode *override_target)
{
  ...
  gumbo_tagset testTagsetArray;
  testTagsetArray[GUMBO_TAG_TABLE] = 1 << GUMBO_NAMESPACE_HTML;
  testTagsetArray[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  testTagsetArray[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  testTagsetArray[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  testTagsetArray[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;

  if (!parser->_parser_state->_foster_parent_insertions ||
      !node_tag_in_set(retval.target, testTagsetArray))
  {
    return retval;
  }
...
static GumboNode *pop_current_node(GumboParser *parser)
{
  ...
  GumboNode *current_node = (GumboNode *)gumbo_vector_pop(&state->_open_elements);
  if (!current_node)
  {
  ...
  {
    int orRestValue = current_node->parse_flags | GUMBO_INSERTION_IMPLICIT_END_TAG;
    current_node->parse_flags = (GumboParseFlags)orRestValue;
  }
...
static void clear_stack_to_table_row_context(GumboParser *parser)
{
  ...
  gumbo_tagset testTagSet;
  testTagSet[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  testTagSet[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;
  testTagSet[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;

  while (!node_tag_in_set(get_current_node(parser), testTagSet))
  {
...
static void clear_stack_to_table_context(GumboParser *parser)
{
  gumbo_tagset testTagSet;
  testTagSet[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  testTagSet[GUMBO_TAG_TABLE] = 1 << GUMBO_NAMESPACE_HTML;
  testTagSet[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;
...
void clear_stack_to_table_body_context(GumboParser *parser)
{
  gumbo_tagset testTagSet;
  testTagSet[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  testTagSet[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  testTagSet[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  testTagSet[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  testTagSet[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;
    ...
    GumboParser *parser, GumboTag tag, GumboParseFlags reason)
{
  ...
  int tempFlags = element->parse_flags | GUMBO_INSERTION_BY_PARSER | reason;

  element->parse_flags = (GumboParseFlags)tempFlags;
...
{
  assert(node->type == GUMBO_NODE_ELEMENT || node->type == GUMBO_NODE_TEMPLATE);
  GumboNode *new_node = (GumboNode *)gumbo_malloc(sizeof(GumboNode));
  *new_node = *node;
  ...
  int tempFlags = new_node->parse_flags & ~GUMBO_INSERTION_IMPLICIT_END_TAG;
  new_node->parse_flags = (GumboParseFlags)tempFlags;

  tempFlags |= reason | GUMBO_INSERTION_BY_PARSER;
  new_node->parse_flags = (GumboParseFlags)tempFlags;
  ...
  for (unsigned int i = 0; i < old_attributes->length; ++i)
  {
    ...
    GumboAttribute *attr = (GumboAttribute *)gumbo_malloc(sizeof(GumboAttribute));
    *attr = *old_attr;
...
static void reconstruct_active_formatting_elements(GumboParser *parser)
{
  ...
  const GumboNode *element = (const GumboNode *)elements->data[i];
  ...
  do
  {
    ...
    // Step 5
    element = (const GumboNode *)elements->data[--i];
...
static void clear_active_formatting_elements(GumboParser *parser)
{
  ...
  do
  {
    node = (const GumboNode *)gumbo_vector_pop(elements);
...
{
  gumbo_tagset tempTagSet;
  tempTagSet[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;

  // #define TAG(tag) [GUMBO_TAG_##tag] = (1 << GUMBO_NAMESPACE_HTML)

  return has_an_element_in_specific_scope(parser, 1, &tag, false, tempTagSet);
}
...
{
  gumbo_tagset tempTagSet;
  tempTagSet[GUMBO_TAG_APPLET] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TABLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_MARQUEE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_OBJECT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_MI] = 1 << GUMBO_NAMESPACE_MATHML;
  tempTagSet[GUMBO_TAG_MO] = 1 << GUMBO_NAMESPACE_MATHML;
  tempTagSet[GUMBO_TAG_MN] = 1 << GUMBO_NAMESPACE_MATHML;
  tempTagSet[GUMBO_TAG_MS] = 1 << GUMBO_NAMESPACE_MATHML;
  tempTagSet[GUMBO_TAG_MTEXT] = 1 << GUMBO_NAMESPACE_MATHML;
  tempTagSet[GUMBO_TAG_ANNOTATION_XML] = 1 << GUMBO_NAMESPACE_MATHML;
  tempTagSet[GUMBO_TAG_FOREIGNOBJECT] = 1 << GUMBO_NAMESPACE_SVG;
  tempTagSet[GUMBO_TAG_DESC] = 1 << GUMBO_NAMESPACE_SVG;
  tempTagSet[GUMBO_TAG_TITLE] = 1 << GUMBO_NAMESPACE_SVG;
...
static bool has_node_in_scope(GumboParser *parser, const GumboNode *node)
{
  ...
  for (int i = open_elements->length; --i >= 0;)
  {
    ...
    if (node_tag_in_set(current, tempTagSet))
    {
      return false;
    }
  }
...
static bool has_an_element_in_scope_with_tagname(GumboParser *parser, int expected_len, const GumboTag expected[])
{
  ...
  return has_an_element_in_specific_scope(parser, expected_len, expected, false, tempTagSet);
}
...
static bool has_an_element_in_list_scope(GumboParser *parser, GumboTag tag)
{
  ...
  tempTagSet[GUMBO_TAG_DESC] = 1 << GUMBO_NAMESPACE_SVG;
  tempTagSet[GUMBO_TAG_TITLE] = 1 << GUMBO_NAMESPACE_SVG;
  tempTagSet[GUMBO_TAG_OL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_UL] = 1 << GUMBO_NAMESPACE_HTML;
...
static bool has_an_element_in_button_scope(GumboParser *parser, GumboTag tag)
{
  ...
  tempTagSet[GUMBO_TAG_DESC] = 1 << GUMBO_NAMESPACE_SVG;
  tempTagSet[GUMBO_TAG_TITLE] = 1 << GUMBO_NAMESPACE_SVG;
  tempTagSet[GUMBO_TAG_BUTTON] = 1 << GUMBO_NAMESPACE_HTML;
...
{
  gumbo_tagset tempTagSet;
  tempTagSet[GUMBO_TAG_OPTGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_OPTION] = 1 << GUMBO_NAMESPACE_HTML;

  return has_an_element_in_specific_scope(parser, 1, &tag, true, tempTagSet);
}
...
{
  gumbo_tagset tempTagSet;
  tempTagSet[GUMBO_TAG_DD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_LI] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_OPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_OPTGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_P] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_RP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_RB] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_RT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_RTC] = 1 << GUMBO_NAMESPACE_HTML;

  for (;
       node_tag_in_set(get_current_node(parser), tempTagSet) &&
...
static void generate_all_implied_end_tags_thoroughly(GumboParser *parser)
{
  gumbo_tagset tempTagSet;
  tempTagSet[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_COLGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_LI] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_OPTION] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  tempTagSet[GUMBO_TAG_RT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_RTC] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_HEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;
...
static bool close_table(GumboParser *parser)
{
  ...
  GumboNode *node = (GumboNode *)pop_current_node(parser);
                             ...
                             GumboTag cell_tag)
{
  ...
  const GumboNode *node = (const GumboNode *)get_current_node(parser);
...
static bool is_special_node(const GumboNode *node)
{
  ...
  gumbo_tagset tempTagSet;
  tempTagSet[GUMBO_TAG_ADDRESS] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_APPLET] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_AREA] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_ARTICLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_ASIDE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_BASE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_BASEFONT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_BGSOUND] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_BLOCKQUOTE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_BR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_BUTTON] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_CENTER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_COL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_COLGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_MENUITEM] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DETAILS] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DIR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DIV] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_DT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_EMBED] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_FIELDSET] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_FIGCAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_FIGURE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_FOOTER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_FORM] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_FRAME] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_FRAMESET] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_H1] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_H2] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_H3] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_H4] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_H5] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_H6] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_HEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_HEADER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_HGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_HR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_IFRAME] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_IMG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_INPUT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_ISINDEX] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_LI] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_LINK] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_LISTING] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_MARQUEE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_MENU] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_META] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_NAV] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_NOEMBED] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_NOFRAMES] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_NOSCRIPT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_OBJECT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_OL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_P] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_PARAM] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_PLAINTEXT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_PRE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_SCRIPT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_SECTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_SELECT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_STYLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_SUMMARY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TABLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TEXTAREA] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_UL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_WBR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagSet[GUMBO_TAG_XMP] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  tempTagSet[GUMBO_TAG_TITLE] = (1 << GUMBO_NAMESPACE_HTML) | (1 << GUMBO_NAMESPACE_SVG);

  // #define TAG_SVG(tag) [GUMBO_TAG_##tag] = (1 << GUMBO_NAMESPACE_SVG)
  // #define TAG_MATHML(tag) [GUMBO_TAG_##tag] = (1 << GUMBO_NAMESPACE_MATHML)

  return node_tag_in_set(node, tempTagSet);
}
    ...
    GumboParser *parser, GumboToken *token, bool is_li)
{
  ...
  for (int i = state->_open_elements.length; --i >= 0;)
  {
    ...
    bool is_list_tag = is_li ? node_html_tag_is(node, GUMBO_TAG_LI) : node_tag_in_set(node, tempTagSet);
    if (is_list_tag)
    {
    ...
    gumbo_tagset tempTagSet2;
    tempTagSet2[GUMBO_TAG_ADDRESS] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagSet2[GUMBO_TAG_DIV] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagSet2[GUMBO_TAG_P] = 1 << GUMBO_NAMESPACE_HTML;

    if (is_special_node(node) &&
        !node_tag_in_set(node, tempTagSet2))
    {
      return;
    }
  }
...
static void adjust_foreign_attributes(GumboToken *token)
{
       ...
       ++i)
  {
    ...
    GumboAttribute *attr = (GumboAttribute *)gumbo_get_attribute(attributes, entry->from);
    if (!attr)
    {
      continue;
    }
...
static bool adoption_agency_algorithm(GumboParser *parser, GumboToken *token, GumboTag subject)
{
  ...
  // Step 1.
  GumboNode *current_node = (GumboNode *)get_current_node(parser);
  ...
  for (unsigned int i = 0; i < 8; ++i)
  {
    ...
    GumboNode *common_ancestor = (GumboNode *)
    ...
    for (int j = 0;;)
    {
      ...
      // Step 13.9.
      int tempFlags = last_node->parse_flags | GUMBO_INSERTION_ADOPTION_AGENCY_MOVED;
      last_node->parse_flags = (GumboParseFlags)tempFlags;
    ...
    // Step 15.
    GumboNode *new_formatting_node = (GumboNode *)clone_node(
        formatting_node, GUMBO_INSERTION_ADOPTION_AGENCY_CLONED);

    int tempFlags2 = formatting_node->parse_flags | GUMBO_INSERTION_IMPLICIT_END_TAG;
    formatting_node->parse_flags = (GumboParseFlags)tempFlags2;
...
static void finish_parsing(GumboParser *parser)
{
       ...
       node = pop_current_node(parser))
  {
    ...
    int tempFlags = node->parse_flags | GUMBO_INSERTION_IMPLICIT_END_TAG;
    node->parse_flags = (GumboParseFlags)tempFlags;
  }
...
static bool handle_in_head(GumboParser *parser, GumboToken *token)
{
  ...
  gumbo_tagset tempTagset2;
  tempTagset2[GUMBO_TAG_IFRAME] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_NOEMBED] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_NOFRAMES] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_STYLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_TEXTAREA] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_TITLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_XMP] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset3;
  tempTagset3[GUMBO_TAG_IFRAME] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset3[GUMBO_TAG_NOEMBED] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset3[GUMBO_TAG_NOFRAMES] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset3[GUMBO_TAG_STYLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset3[GUMBO_TAG_TEXTAREA] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset3[GUMBO_TAG_TITLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset3[GUMBO_TAG_XMP] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset4;
  tempTagset4[GUMBO_TAG_NOFRAMES] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_STYLE] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset5;
  tempTagset5[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset5[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset5[GUMBO_TAG_BR] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  else if (tag_in(token, kStartTag, tempTagset))
  {
    insert_element_from_token(parser, token);
    pop_current_node(parser);
  ...
  else if (tag_is(token, kStartTag, GUMBO_TAG_META))
  {
    ...
    // XHTML5 Parser support for cdata and rcdata in head to fix <title/> and etc
           ...
           tag_in(token, kStartTag, tempTagset2))
  {
    insert_element_from_token(parser, token);
    return true;
  }
  else if (parser->_options->use_xhtml_rules &&
           tag_in(token, kEndTag, tempTagset3))
  {
    pop_current_node(parser);
    return true;
  }
  ...
  else if (tag_in(token, kStartTag, tempTagset4))
  {
  ...
  else if (tag_in(token, kEndTag, tempTagset5))
  {
    pop_current_node(parser);
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_AFTER_HEAD);
    parser->_parser_state->_reprocess_current_token = true;
...
static bool handle_in_head_noscript(GumboParser *parser, GumboToken *token)
{
  ...
  gumbo_tagset tempTagset2;
  tempTagset2[GUMBO_TAG_HEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_NOSCRIPT] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  else if (tag_is(token, kEndTag, GUMBO_TAG_NOSCRIPT))
  {
    ...
    return true;
           ...
           tag_in(token, kStartTag, tempTagset))
  {
    return handle_in_head(parser, token);
  }
  else if (tag_in(token, kStartTag, tempTagset2) ||
...
static bool handle_after_head(GumboParser *parser, GumboToken *token)
{
  ...
  tempTagset[GUMBO_TAG_STYLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset[GUMBO_TAG_TITLE] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset2;
  tempTagset2[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_BR] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  else if (tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE))
  {
    return handle_in_head(parser, token);
           ...
           (token->type == GUMBO_TOKEN_END_TAG &&
            !tag_in(token, kEndTag, tempTagset2)))
  {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
...
static void free_node(GumboNode *node_to_free)
{
  ...
  while (node != NULL)
  {
    ...
    case GUMBO_NODE_ELEMENT:
    {
      ...
      {
        gumbo_destroy_attribute((GumboAttribute *)node->v.element.attributes.data[i]);
      }
...
static bool handle_in_body(GumboParser *parser, GumboToken *token)
{
  ...
  gumbo_tagset tempTagset2;
  tempTagset2[GUMBO_TAG_DD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_DT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_LI] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_P] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset2[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset3;
  tempTagset3[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset3[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset4;
  tempTagset4[GUMBO_TAG_ADDRESS] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_ARTICLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_ASIDE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_BLOCKQUOTE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_CENTER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_DETAILS] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_DIR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_DIV] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_DL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_FIELDSET] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_FIGCAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_FIGURE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_FOOTER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_HEADER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_HGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_MENU] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_MAIN] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_NAV] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_OL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_P] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_SECTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_SUMMARY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset4[GUMBO_TAG_UL] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset5;
  tempTagset5[GUMBO_TAG_H1] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset5[GUMBO_TAG_H2] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset5[GUMBO_TAG_H3] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset5[GUMBO_TAG_H4] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset5[GUMBO_TAG_H5] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset5[GUMBO_TAG_H6] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset6;
  tempTagset6[GUMBO_TAG_PRE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset6[GUMBO_TAG_LISTING] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset7;
  tempTagset7[GUMBO_TAG_DD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset7[GUMBO_TAG_DT] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset8;
  tempTagset8[GUMBO_TAG_ADDRESS] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_ARTICLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_ASIDE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_BLOCKQUOTE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_BUTTON] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_CENTER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_DETAILS] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_DIR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_DIV] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_DL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_FIELDSET] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_FIGCAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_FIGURE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_FOOTER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_HEADER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_HGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_LISTING] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_MAIN] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_MENU] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_NAV] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_OL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_PRE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_SECTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_SUMMARY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset8[GUMBO_TAG_UL] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset9;
  tempTagset9[GUMBO_TAG_B] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_BIG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_CODE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_EM] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_FONT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_I] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_S] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_SMALL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_STRIKE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_STRONG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_TT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset9[GUMBO_TAG_U] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset10;
  tempTagset10[GUMBO_TAG_A] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_B] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_BIG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_CODE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_EM] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_FONT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_I] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_NOBR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_S] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_SMALL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_STRIKE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_STRONG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_TT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset10[GUMBO_TAG_U] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset11;
  tempTagset11[GUMBO_TAG_APPLET] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset11[GUMBO_TAG_MARQUEE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset11[GUMBO_TAG_OBJECT] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset12;
  tempTagset12[GUMBO_TAG_AREA] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset12[GUMBO_TAG_BR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset12[GUMBO_TAG_EMBED] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset12[GUMBO_TAG_IMG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset12[GUMBO_TAG_IMAGE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset12[GUMBO_TAG_KEYGEN] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset12[GUMBO_TAG_WBR] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset13;
  tempTagset13[GUMBO_TAG_PARAM] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset13[GUMBO_TAG_SOURCE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset13[GUMBO_TAG_TRACK] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset14;
  tempTagset14[GUMBO_TAG_IFRAME] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset14[GUMBO_TAG_NOEMBED] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset14[GUMBO_TAG_TEXTAREA] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset14[GUMBO_TAG_XMP] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset15;
  tempTagset15[GUMBO_TAG_OPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset15[GUMBO_TAG_OPTGROUP] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset16;
  tempTagset16[GUMBO_TAG_RB] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset16[GUMBO_TAG_RP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset16[GUMBO_TAG_RT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset16[GUMBO_TAG_RTC] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagset17;
  tempTagset17[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_COL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_COLGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_FRAME] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_HEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagset17[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  else if (tag_in(token, kStartTag, tempTagset) || tag_is(token, kEndTag, GUMBO_TAG_TEMPLATE))
  {
    return handle_in_head(parser, token);
  }
  else if (tag_is(token, kStartTag, GUMBO_TAG_BODY))
  {
    ...
    if (state->_open_elements.length < 2 ||
        !node_html_tag_is((GumboNode *)state->_open_elements.data[1], GUMBO_TAG_BODY) || has_open_element(parser, GUMBO_TAG_TEMPLATE))
    {
      ignore_token(parser);
      return false;
    }
    state->_frameset_ok = false;
    merge_attributes(token, (GumboNode *)state->_open_elements.data[1]);
    return false;
  }
  else if (tag_is(token, kStartTag, GUMBO_TAG_FRAMESET))
  {
  ...
  else if (token->type == GUMBO_TOKEN_EOF)
  {
    for (unsigned int i = 0; i < state->_open_elements.length; ++i)
    {
      if (!node_tag_in_set((GumboNode *)state->_open_elements.data[i], tempTagset2))
      {
        parser_add_parse_error(parser, token);
      }
    }
  ...
  else if (tag_in(token, kEndTag, tempTagset3))
  {
    ...
    for (unsigned int i = 0; i < state->_open_elements.length; ++i)
    {
      gumbo_tagset tempTagsetTag;
      tempTagsetTag[GUMBO_TAG_DD] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_DT] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_LI] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_OPTGROUP] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_OPTION] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_P] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_RB] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_RP] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_RT] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_RTC] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
      tempTagsetTag[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;

      if (!node_tag_in_set((GumboNode *)state->_open_elements.data[i], tempTagsetTag))
      {
        parser_add_parse_error(parser, token);
        success = false;
        break;
  ...
  else if (tag_in(token, kStartTag, tempTagset5))
  {
    gumbo_tagset tempTagsetTag;
    tempTagsetTag[GUMBO_TAG_H1] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagsetTag[GUMBO_TAG_H2] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagsetTag[GUMBO_TAG_H3] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagsetTag[GUMBO_TAG_H4] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagsetTag[GUMBO_TAG_H5] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagsetTag[GUMBO_TAG_H6] = 1 << GUMBO_NAMESPACE_HTML;

    bool result = maybe_implicitly_close_p_tag(parser, token);
    if (node_tag_in_set(get_current_node(parser), tempTagsetTag))
    {
      parser_add_parse_error(parser, token);
      pop_current_node(parser);
      result = false;
  ...
  else if (tag_in(token, kStartTag, tempTagset6))
  {
    bool result = maybe_implicitly_close_p_tag(parser, token);
    insert_element_from_token(parser, token);
  ...
  else if (tag_in(token, kStartTag, tempTagset7))
  {
  ...
  else if (tag_in(token, kEndTag, tempTagset8))
  {
  ...
  else if (tag_in(token, kEndTag, tempTagset7))
  {
  ...
  {
    GumboTag tempEnumTagArray[] = {GUMBO_TAG_H1, GUMBO_TAG_H2, GUMBO_TAG_H3, GUMBO_TAG_H4, GUMBO_TAG_H5, GUMBO_TAG_H6};
    if (!has_an_element_in_scope_with_tagname(parser, 6, tempEnumTagArray))
    {
    ...
    else
    {
      ...
      } while (!node_tag_in_set(current_node, tempTagset5));
      return success;
    }
  }
  ...
  else if (tag_in(token, kStartTag, tempTagset9))
  {
    reconstruct_active_formatting_elements(parser);
    add_formatting_element(parser, insert_element_from_token(parser, token));
    return true;
  ...
  else if (tag_in(token, kEndTag, tempTagset10))
  {
    return adoption_agency_algorithm(parser, token, token->v.end_tag);
  }
  else if (tag_in(token, kStartTag, tempTagset11))
  {
    reconstruct_active_formatting_elements(parser);
    insert_element_from_token(parser, token);
    add_formatting_element(parser, &kActiveFormattingScopeMarker);
  ...
  else if (tag_in(token, kEndTag, tempTagset11))
  {
    GumboTag token_tag = token->v.end_tag;
  ...
  else if (tag_in(token, kStartTag, tempTagset12))
  {
    bool success = true;
    ...
    if (tag_is(token, kStartTag, GUMBO_TAG_IMAGE))
    {
      ...
      int tempFlags = node->parse_flags | GUMBO_INSERTION_FROM_IMAGE;
      node->parse_flags = (GumboParseFlags)tempFlags;
    }
    pop_current_node(parser);
    acknowledge_self_closing_tag(parser);
  ...
  else if (tag_in(token, kStartTag, tempTagset13))
  {
    insert_element_from_token(parser, token);
    pop_current_node(parser);
    acknowledge_self_closing_tag(parser);
  ...
  else if (tag_is(token, kStartTag, GUMBO_TAG_ISINDEX))
  {
    ...
    GumboAttribute *name = (GumboAttribute *)gumbo_malloc(sizeof(GumboAttribute));
    ...
    // XHTML5 Parser support in body to fix <iframe/> and related non-void self-closing tags
           ...
           tag_in(token, kStartTag, tempTagset14))
  {
    gumbo_tagset tempTagsetTag;
    tempTagsetTag[GUMBO_TAG_IFRAME] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagsetTag[GUMBO_TAG_TEXTAREA] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagsetTag[GUMBO_TAG_XMP] = 1 << GUMBO_NAMESPACE_HTML;

    if (tag_in(token, kStartTag, tempTagsetTag))
    {
      set_frameset_not_ok(parser);
    }
    insert_element_from_token(parser, token);
    return true;
           ...
           tag_in(token, kEndTag, tempTagset14))
  {
    pop_current_node(parser);
    return true;
  }
  ...
  else if (tag_in(token, kStartTag, tempTagset15))
  {
  ...
  else if (tag_in(token, kStartTag, tempTagset16))
  {
    gumbo_tagset tempTagsetTag;
    tempTagsetTag[GUMBO_TAG_RT] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagsetTag[GUMBO_TAG_RP] = 1 << GUMBO_NAMESPACE_HTML;
    ...
    bool success = true;
    GumboTag exception = tag_in(token, kStartTag, tempTagsetTag) ? GUMBO_TAG_RTC : GUMBO_TAG_LAST;
  ...
  else if (tag_in(token, kStartTag, tempTagset17))
  {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
...
{
  gumbo_tagset tempTagsetTag1;
  tempTagsetTag1[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagsetTag2;
  tempTagsetTag2[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_COL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_COLGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagsetTag3;
  tempTagsetTag3[GUMBO_TAG_STYLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_SCRIPT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  else if (tag_in(token, kStartTag, tempTagsetTag1))
  {
    gumbo_tagset tempTagset;
    tempTagset[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
    tempTagset[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
    ...
    if (tag_in(token, kStartTag, tempTagset))
    {
      insert_element_of_tag_type(
  ...
  else if (tag_in(token, kEndTag, tempTagsetTag2))
  {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
  }
  else if (tag_in(token, kStartTag, tempTagsetTag3) ||
...
{
  gumbo_tagset tempTagsetTag1;
  tempTagsetTag1[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_COL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_COLGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
...
static bool handle_in_table_body(GumboParser *parser, GumboToken *token)
{
  ...
  gumbo_tagset tempTagsetTag3;
  tempTagsetTag3[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_COL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_COLGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagsetTag4;
  tempTagsetTag4[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_COL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_COLGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  else if (tag_in(token, kEndTag, tempTagsetTag4))
  {
    parser_add_parse_error(parser, token);
    ignore_token(parser);
    return false;
...
static bool handle_in_row(GumboParser *parser, GumboToken *token)
{
  ...
  if (tag_in(token, kStartTag, tempTagsetTag1))
  {
  ...
  else if (tag_in(token, kStartTag, tempTagsetTag2) || tag_is(token, kEndTag, GUMBO_TAG_TABLE))
  {
    if (!has_an_element_in_table_scope(parser, GUMBO_TAG_TR))
    {
      parser_add_parse_error(parser, token);
  ...
  else if (tag_in(token, kEndTag, tempTagsetTag3))
  {
    if (!has_an_element_in_table_scope(parser, token->v.end_tag) ||
...
static bool handle_in_cell(GumboParser *parser, GumboToken *token)
{
  ...
  gumbo_tagset tempTagsetTag3;
  tempTagsetTag3[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_COL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_COLGROUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_HTML] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagsetTag4;
  tempTagsetTag4[GUMBO_TAG_TABLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag4[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;

  if (tag_in(token, kEndTag, tempTagsetTag1))
  {
    GumboTag token_tag = token->v.end_tag;
    if (!has_an_element_in_table_scope(parser, token_tag))
    {
  ...
  else if (tag_in(token, kStartTag, tempTagsetTag2))
  {
...
{
  gumbo_tagset tempTagsetTag1;
  tempTagsetTag1[GUMBO_TAG_INPUT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_KEYGEN] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TEXTAREA] = 1 << GUMBO_NAMESPACE_HTML;

  gumbo_tagset tempTagsetTag2;
  tempTagsetTag2[GUMBO_TAG_SCRIPT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag2[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;
...
static bool handle_in_select_in_table(GumboParser *parser, GumboToken *token)
{
  gumbo_tagset tempTagsetTag1;
  tempTagsetTag1[GUMBO_TAG_CAPTION] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TABLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TBODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TFOOT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_THEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TR] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  else if (tag_in(token, kEndTag, tempTagsetTag1))
  {
    parser_add_parse_error(parser, token);
    if (!has_an_element_in_table_scope(parser, token->v.end_tag))
    {
...
{
  gumbo_tagset tempTagsetTag1;
  tempTagsetTag1[GUMBO_TAG_BASE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_BASEFONT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_BGSOUND] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_LINK] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_META] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_NOFRAMES] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_SCRIPT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_STYLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TEMPLATE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TITLE] = 1 << GUMBO_NAMESPACE_HTML;
  ...
  gumbo_tagset tempTagsetTag3;
  tempTagsetTag3[GUMBO_TAG_TD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag3[GUMBO_TAG_TH] = 1 << GUMBO_NAMESPACE_HTML;
...
static bool handle_in_foreign_content(GumboParser *parser, GumboToken *token)
{
  ...
  gumbo_tagset tempTagsetTag1;
  tempTagsetTag1[GUMBO_TAG_B] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_BIG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_BLOCKQUOTE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_BODY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_BR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_CENTER] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_CODE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_DD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_DIV] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_DL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_DT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_EM] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_EMBED] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_H1] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_H2] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_H3] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_H4] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_H5] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_H6] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_HEAD] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_HR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_I] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_IMG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_LI] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_LISTING] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_MENU] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_META] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_NOBR] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_OL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_P] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_PRE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_RUBY] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_S] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_SMALL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_SPAN] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_STRONG] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_STRIKE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_SUB] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_SUP] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TABLE] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_TT] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_U] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_UL] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_VAR] = 1 << GUMBO_NAMESPACE_HTML;
...
static bool handle_token(GumboParser *parser, GumboToken *token)
{
  ...
  gumbo_tagset tempTagsetTag1;
  tempTagsetTag1[GUMBO_TAG_MGLYPH] = 1 << GUMBO_NAMESPACE_HTML;
  tempTagsetTag1[GUMBO_TAG_MALIGNMARK] = 1 << GUMBO_NAMESPACE_HTML;
      ...
      (is_mathml_integration_point(current_node) &&
       (token->type == GUMBO_TOKEN_CHARACTER ||
        ...
        (token->type == GUMBO_TOKEN_START_TAG &&
         !tag_in(token, kStartTag, tempTagsetTag1)))) ||
    ...
    const GumboTag fragment_ctx, const GumboNamespaceEnum fragment_namespace)
{
  ...
  do
  {
    ...
    if (parser._options->use_xhtml_rules)
    {
      ...
      {
        gumbo_tagset testTagsetArray;
        testTagsetArray[GUMBO_TAG_AREA] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_BASE] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_BASEFONT] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_BGSOUND] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_BR] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_COL] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_EMBED] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_FRAME] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_HR] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_IMG] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_INPUT] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_ISINDEX] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_KEYGEN] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_LINK] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_META] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_PARAM] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_SOURCE] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_SPACER] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_TRACK] = 1 << GUMBO_NAMESPACE_HTML;
        testTagsetArray[GUMBO_TAG_WBR] = 1 << GUMBO_NAMESPACE_HTML;

        if (!tag_in(&token, true, testTagsetArray))
        {
          inject_end = true;
...
void gumbo_destroy_output(GumboOutput *output)
{
  ...
  {
    gumbo_error_destroy((GumboError *)output->errors.data[i]);
  }
