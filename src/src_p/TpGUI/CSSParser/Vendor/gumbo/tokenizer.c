...
static StateResult emit_current_tag(GumboParser* parser, GumboToken* output) {
  ...
  if (tag_state->_is_start_tag) {
    ...
    for (unsigned int i = 0; i < tag_state->_attributes.length; ++i) {
      gumbo_destroy_attribute((GumboAttribute*)tag_state->_attributes.data[i]);
    }
...
void gumbo_tokenizer_state_init(
  ...
  GumboTokenizerState* tokenizer = (GumboTokenizerState*)gumbo_malloc(sizeof(GumboTokenizerState));
...
static StateResult handle_char_ref_in_data_state(
    ...
    int c, GumboToken* output) {
  gumbo_tokenizer_set_state(parser, GUMBO_LEX_DATA);
  return (StateResult)emit_char_ref(parser, ' ', false, output);
}
...
static StateResult handle_rcdata_state(
  ...
  switch (c) {
    ...
    case -1:
      return (StateResult)emit_eof(parser, output);
    default:
      return (StateResult)emit_current_char(parser, output);
  }
}
...
static StateResult handle_tag_name_state(
  ...
  switch (c) {
    ...
    case '>':
      finish_tag_name(parser);
      gumbo_tokenizer_set_state(parser, GUMBO_LEX_DATA);
      return (StateResult)emit_current_tag(parser, output);
    case '\0':
      tokenizer_add_parse_error(parser, GUMBO_ERR_UTF8_NULL);
